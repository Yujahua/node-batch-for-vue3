(function (window, module) {
    ...
    vBat.fn = vBat.prototype = { version };
    ...
    var fs = {
        options: {
            ignore: [],     // 目录下忽略的文件或路径
            ondebug: true,  // 默认开启debug，日志内容输出在debug.log文件，错误会显示在error.log文件 [!未启用]
            rootDir: "../../origin",  // 待扫描的本地磁盘根目录，默认是@origin
            silent: false,  // 静默模式，开启后不打印日志
            undressMap: {   // 执行文件扫描时得到的关键内容，收集
                "inputFileList": [],    // 输入文件名存档
                "outputFileList": [],   // 输出文件名存档 [!未启用]
    ...
    function readDir_R(path, file, code) {
        ...
        if (fs.isExists(path)) {
            if (fs.isDir(path)) {
                ...
                if (fs.isEntranceIndexFile(path)) {//过滤mod后缀的文件
